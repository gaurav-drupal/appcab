...
var channels = {},
    ...
    settingsDefaults = {
      ...
      serviceKey: '',
      debug: false,
      baseAuthPath: '/home/blackcr/nodejs/',
      publishUrl: 'publish',
 ...
 * todo: make this actually call the callback.php
 */
var sendCallbackToBackend = function (message, callback) {
  var requestBody = querystring.stringify({
    messageJson: JSON.stringify(message),
    serviceKey: settings.serviceKey
  });
  ...
  if (settings.debug) {
    console.log("Sending callback to backend", message, options);
  }
  request.post(options, callback);
}
...
var publishMessageToChannel = function (message) {
  var callbackDelay = 15000;
  if(message['callbackDelay'] != undefined && message['callbackDelay'] != null) {
    callbackDelay = message['callbackDelay'];
  }
  if(message['resendCallback'] != undefined && message['resendCallback'] != null) {
    var resendCallbackURL = settings.backend.scheme + '://' + settings.backend.host + ':' + settings.backend.port + message['resendCallback'];
    var resendCallback = function(resendCallbackURL) {
      console.log('I am here!');
      setTimeout(function() {
        var wget = 'wget -O /dev/null "' + resendCallbackURL + '"';
        var exec = require('child_process').exec;
        exec(wget, function(error, stdout, stderr) {
          if (error) {
    ...
    resendCallback(resendCallbackURL);

    if (settings.debug) {
      if (message['resendCallback'] != undefined && message['resendCallback'] != null) {
        console.log('Resend Callback = ', resendCallbackURL);
      } else {
        console.log('Resend Callback not set.')
      }
    }
  }
...
console.log(settings.baseAuthPath + settings.publishUrl);
console.log('Started ' + settings.scheme + ' server:'+settings.port);
