...
<div id="container">
  ...
  <br/>
  <noscript>
    <div class="error"><em>You have Javascript disabled. Please enable it to use the proxy</em></div>
  </noscript>
<?php
...
if (isset($_GET['action'])) {
  if ($_GET['action'] == 'cookies') {
    ...
    if ($_GET['action'] == 'auth' && isset($_GET['server'], $_GET['realm'], $_GET['auth_url'])) {
      ...
      echo '<tr><td class="option">User name</td><td class="option"><input type="text" name="username" value="" /></td></tr>';
      echo '<tr><td class="option">Password</td><td class="option"><input type="password" name="password" value="" /></td></tr>';
  ...
  ?>
  <form name="<?php echo $PHProxy->config['proxy_url_form_name'] ?>" method="get"
  ...
  <form name="<?php echo $PHProxy->config['proxy_settings_form_name'] ?>" method="get" action=""
        onsubmit="return submit_form();">
    <table style="width: 100%">
      <tr>
        <td class="option" style="width: 20%">URL</td>
        <td class="option" style="width: 80%">&nbsp;<label>
            <input type="text" name="url" size="70" value=""/>
          </label></td>
      </tr>
      <?php echo $PHProxy->options_list(TRUE, TRUE) ?>
      <tr>
        <td class="option" style="width: 20%">New Window</td>
        <td class="option" style="width: 80%"><label>
    ...
    <div style="text-align: center"><input type="submit" name="browse" value="Browse" onclick="return submit_form();"/>
    </div>
    <div style="text-align: center">PHProxy <?php echo $PHProxy->version ?> &copy; 2011 Capiofiles.com
      <br/>
      <strong>DON CAPRIO INC.</strong>

      <br/>

      http://www.capriofiles.com
      <br/>http://www.doncaprio.com
      <br/>http://wap.capriofiles.com
      <br/>

      <strong>caprio @ capriofiles.com</strong></div>
  </form>
  </div>
<?php
}
