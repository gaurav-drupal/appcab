...
class PHProxy {
  ...
  function set_response() {
    ...
    if ($this->flags['accept_cookies'] == 1) {
      //$this->set_cookies(); // don't set cookies ever
    }
    ...
    if ($this->follow_location()) {
      ...
      if (!in_array($this->content_type, array(
        ...
        'text/css'
      )) && (!$this->content_length || (int) $this->content_length <= $this->config['max_file_size'])
      ) {
  ...
  function http_basic_auth() {
    if (empty($this->response_code)) {
      if (isset($this->url_segments['user'], $this->url_segments['pass'])) {
        ...
        if (($stored_auth = $this->get_cookies('AUTH')) != '') {
      ...
      if ($this->response_code == 401 && preg_match('#www-authenticate:\s*basic\s+(?:realm="(.*?)")?#i', $this->response_headers, $matches)) {
      ...
      switch ($tag) {
        ...
        case 'applet':
          ...
          if (isset($attrs['archive'])) {
            $rebuild = TRUE;
            $attrs['archive'] = implode(',', array_map(array(
              &$this,
              'proxify_url'
            ), preg_split('#\s*,\s*#', $attrs['archive'])));
          }
          if (!empty($temp)) {
            $this->base = $temp;
          }
  ...
  function set_flags($flags) {
    if (is_numeric($flags)) {
      ...
      if (isset($_COOKIE['flags']) && is_numeric($_COOKIE['flags']) && strlen($_COOKIE['flags']) == count($this->flags)) {
  ...
  function set_post_files($array, $parent_key = NULL) {
    ...
    foreach ($array as $key => $value) {
      ...
      if (is_array($value)) {
        ...
        if (preg_match('#^([^\[\]]+)\[(name|type|tmp_name)\]#', $key, $m)) {
  ...
  function options_list($tabulate = FALSE, $comments_on = FALSE) {
    ...
    foreach ($this->flags as $flag_code => $flag_status) {
      $interface = array(
        $comments[$flag_code][0],
        ' <input type="checkbox" name="ops[]"' . ($flag_status ? ' checked="checked"' : '') . ' /> '
      );
